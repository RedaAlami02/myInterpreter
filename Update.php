  ...
  <div class="update-wrap animate-up">
    ...
    <div class="mb-4" style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:12px">
      ...
      <a href="handlers/sync_idb.php" class="btn btn-ghost btn-sm" target="_blank"
         style="align-self:center" title="Enrichir toutes les sociétés depuis fundamentals.example">
      ...
      <div class="field-group mb-4">
        <div class="field-group-title cyan" style="display:flex;align-items:center;justify-content:space-between">
          ...
          <button type="button" id="btnAutoFill" class="btn btn-ghost btn-sm" onclick="autoFill()" disabled>
            <i class="fas fa-magic"></i> Auto-remplir depuis the fundamentals provider
          </button>
        </div>
        <?php
...
async function autoFill() {
    ...
    try {
        const r = await fetch('handlers/the fundamentals provider_proxy.php?symbol=' + encodeURIComponent(sym));
        ...
        btn.innerHTML = '<i class="fas fa-check t-emerald"></i> Rempli depuis the fundamentals provider';
        window.showToast('Champs pré-remplis depuis the fundamentals provider (' + (d.idb_name || sym) + ')', 'success');
    } catch (e) {
        btn.innerHTML = '<i class="fas fa-magic"></i> Auto-remplir depuis the fundamentals provider';
        btn.disabled  = false;
        window.showToast('Erreur the fundamentals provider : ' + e.message, 'error');
    }
}
</script>
</body>
