<?php
/**
 * Bulk sync all companies from fundamentals.example.
...
set_time_limit(600); // 10 min — the fundamentals provider calls take time
...
<head>
  <meta charset="UTF-8">
  <title>Sync the fundamentals provider</title>
...
<body>
<h2>🔄 Sync the fundamentals provider <?= $dryRun ? '<span style="color:var(--warn)">(DRY RUN)</span>' : '' ?></h2>
<p style="color:var(--text-dim);margin-bottom:24px">
  Mise à jour de toutes les sociétés depuis fundamentals.example.
...
foreach ($companies as $co) {
    ...
    // Call the fundamentals provider proxy inline (same logic, direct function call)
    $proxy_url = BASE_URL . '/handlers/the fundamentals provider_proxy.php?symbol=' . urlencode($symbol);
    ...
    flush();
    usleep(200_000); // 200ms between companies to avoid hammering the fundamentals provider
}
?>
