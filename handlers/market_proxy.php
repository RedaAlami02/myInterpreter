...
function mkt_fetch_symbol(string $symbol): array {
    ...
    if (!$ext_name) return ['error' => 'symbol not found in registry'];
    ...
    $rnpg = mkt_latest($s2,'rnpg'); $fp  = mkt_latest($s2,'fp');  $nmt = mkt_latest($s2,'nmt');
    ...
    $description = $desc_d ? (mb_substr(trim($desc_d['description']??''),0,3800) ?: null) : null;
...
// ── Run as HTTP endpoint when called directly ─────────────────────────────────
if (basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    ...
    echo json_encode(mkt_fetch_symbol($symbol), JSON_UNESCAPED_UNICODE|JSON_PARTIAL_OUTPUT_ON_ERROR);
}
