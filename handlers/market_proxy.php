<?php
/**
 * fundamentals.example API proxy — server-side to avoid CORS.
...
header('Cache-Control: public, max-age=3600'); // cache 1h — the fundamentals provider data is daily
...
function idb_batch(array $requests): array {
    ...
    foreach ($requests as $key => $spec) {
        ...
        $hdr = ['Accept: */*', 'User-Agent: Mozilla/5.0 (compatible)',
                'Origin: https://fundamentals.example', 'Referer: https://fundamentals.example/'];
...
$SB = 'https://fundamentals.example/api/proxy/supabase';

// ── Step 1: stock info + financial summary + symbol list ─────────────────────
$s1 = idb_batch([
    'stock'     => ['url' => "https://fundamentals.example/api/proxy/stock/{$symbol}"],
    'financial' => ['url' => "https://fundamentals.example/api/proxy/financial/{$symbol}"],
    'symlinks'  => ['url' => "https://fundamentals.example/api/proxy/symblinks"],
]);
...
// Find the fundamentals provider name + sector for this symbol
...
// BPA — the fundamentals provider financial endpoint first, then compute
$bpa = null;
