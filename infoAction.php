...
if (!$dbError && $_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['NAME'])) {
    ...
    try {
        ...
        if ($company) {
            ...
            // Get symbol for the fundamentals provider enrichment
      ...
      <?php else: ?>
        <div class="search-result">
          ...
          <?php if (!empty($history)): ?>
            ...
            <?php if ($symbol): ?>
            <!-- the fundamentals provider enrichment (JS-loaded) -->
...
(async function() {
  ...
  try {
    ...
    container.innerHTML = html || '<div style="color:var(--text-faint);font-size:13px;padding:12px">Aucune donnée the fundamentals provider disponible pour cette société.</div>';
    ...
    document.getElementById('idb-enrichment').innerHTML =
      `<div style="color:var(--text-faint);font-size:13px;padding:12px">Données the fundamentals provider indisponibles.</div>`;
  }
})();
</script>
<?php endif; ?>
